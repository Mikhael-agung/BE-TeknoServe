...
class TeknisiController {
  ...
  static async getDashboardStats(req, res) {
    try {
      ...
      const progressCount = await Complaint.countByTeknisiId(
        userId,
        "on_progress"
      );
      const completedCount = await Complaint.countByTeknisiId(
        userId,
        "completed"
      );
      ...
      res
        .status(500)
        .json(errorResponse("Gagal mengambil dashboard stats", 500));
    }
  }
  ...
  static async getReadyComplaints(req, res) {
    try {
      ...
      res
        .status(500)
        .json(errorResponse("Gagal mengambil komplain ready", 500));
    }
  }
  ...
  static async getProgressComplaints(req, res) {
    try {
      ...
      res
        .status(500)
        .json(errorResponse("Gagal mengambil komplain progress", 500));
    }
  }
  ...
  static async getCompletedComplaints(req, res) {
    try {
      ...
      res
        .status(500)
        .json(errorResponse("Gagal mengambil komplain completed", 500));
    }
  }
  ...
  static async takeComplaint(req, res) {
    try {
      ...
      if (findError || !complaint) {
        ...
        return res
          .status(404)
          .json(errorResponse("Komplain tidak ditemukan", 404));
      }
      ...
      if (complaint.status !== "complaint") {
        return res
          .status(400)
          .json(errorResponse("Komplain sudah dalam proses", 400));
      }

      if (complaint.teknisi_id) {
        return res
          .status(400)
          .json(errorResponse("Komplain sudah diambil teknisi lain", 400));
      }
      ...
      await supabase.from("complaint_statuses").insert([
        {
          complaint_id: id,
          status: "on_progress",
          teknisi_id: userId,
      ...
      res.json(successResponse(updatedComplaint, "Komplain berhasil diambil"));
    } catch (error) {
      console.error("Take complaint error:", error);
      res.status(500).json(errorResponse("Gagal mengambil komplain", 500));
    }
  }
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json(
            errorResponse(
              "Komplain sudah selesai dengan catatan penyelesaian. Tidak dapat diupdate lagi.",
              400
            )
          );
      }
      ...
      if (recentEntries && recentEntries.length > 0) {
        ...
        if (isSameReason || isSimilarResolution) {
          ...
          return res
            ...
            .json(
              errorResponse(
                "Status sudah diupdate baru-baru ini. Silakan tunggu beberapa menit.",
                400
              )
            );
        }
      ...
      if (status === "completed" && resolution_notes) {
        ...
        console.log(
          `💾 Saving resolution notes: ${resolution_notes.substring(0, 50)}...`
        );
      }
      ...
      res
        .status(500)
        .json(errorResponse("Gagal mengupdate status komplain", 500));
    }
  }
}
