...
class ComplaintController {
  static async create(req, res) {
    try {
      ...
      if (!judul || !kategori) {
        ...
        return res
          .status(400)
          .json(errorResponse("Judul dan kategori wajib diisi", 400));
      }

      if (!alamat || alamat.trim() === "") {
        console.log("❌ Validation failed: alamat missing or empty");
        return res.status(400).json(errorResponse("Alamat wajib diisi", 400));
      }
  ...
  static async getHistory(req, res) {
    try {
      ...
      console.log("🔎 userId yang dipakai filter:", req.user.id);
      ...
      const { data: complaints, total } = await Complaint.findByUserId(
        userId,
        filters
      );
      ...
      res
        .status(500)
        .json(errorResponse("Gagal mengambil history komplain", 500));
    }
  }
  ...
  static async getDetail(req, res) {
    try {
      ...
      if (
        ...
        !["admin", "teknisi"].includes(req.user.role)
      ) {
        return res
          .status(403)
          .json(
      ...
      res
        .status(500)
        .json(errorResponse("Gagal mengambil detail komplain", 500));
    }
  }
    ...
    try {
      ...
      if (!["teknisi", "admin"].includes(userRole)) {
        return res
          ...
          .json(
            errorResponse(
              "Hanya teknisi atau admin yang dapat mengupdate status",
              403
            )
          );
      }
  ...
  static async getStatusHistory(req, res) {
    try {
      ...
      res
        .status(500)
        .json(errorResponse("Gagal mengambil riwayat status", 500));
    }
  }
}
